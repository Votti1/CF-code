...
colors = [(216, 211, 200), (3, 12, 19), (37, 47, 56), (75, 88, 98), (174, 177, 181)]
...
def print_into_img(pixels, matrix, scale=11):
    ...
    for i in range(len(matrix)):
        shift = 0 + (0 if (i + 1) % 8 >= 4 else 3)
        for j in range(len(matrix)):
            if matrix[i][j] == 0: 
                color = colors[matrix[i][j]]
            else: 
                color = colors[matrix[i][j] if shift < 3 else abs(5 - matrix[i][j])]
                shift = (shift + 1) % 6
...
def draw_corner(pixels, size, scale=11):
    global colors
    edge_color = colors[1:3]
    for edge in range(4):
        for i in range(size):
            j = 0 if edge >= 2 else size - 1
            color = edge_color[math.ceil(randint(0,100)/70) - 1]
            for y in range(scale):
                for x in range(scale):
                    x_start = scale * (j if edge % 2 == 0 else i)
                    y_start = scale * (j if edge % 2 == 1 else i)
                    pixels[x_start + x, y_start + y] = color
    for i, j in [(3, 2), (2, 2), (3, 1)]:
        for y in range(scale):
                for x in range(scale):
                    pixels[scale * (size - i) + x, scale * j + y] = colors[4] if i - 1 == j else colors[3]

    for i, j in [(2, 3), (1, 3), (2, 2)]:
        for y in range(scale):
                for x in range(scale):
                    pixels[scale * i + x, scale * (size - j) + y] = colors[4] if i + 1 != j else colors[3]
    

def main():
    scale = 11
    word = input().encode('utf-8').hex(" ").split()
    size = math.ceil(math.sqrt(len(word))) * 4 + 5
    word = transalte(word)
